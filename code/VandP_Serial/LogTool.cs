...
namespace Build_framework
{
    ...
    public class LogTool
    {
        ...
        public static void UseSensibleDefaults(eloglevel logLevel)
        {
             ...
             * 
            */
            string[] logFile = DefaultLogToolAndLocation();
        ...
        public static void UseSensibleDefaults(string logFileName, string logLocation, eloglevel logLevel)
        {
            ...
            */
            LogTool.Profile = elogprofile.primary;
            //I prefer a simple text file log but you can change this 
            //to use the event log also.
            LogTool.LogWhere = elogwhere.file;

            if (Convert.ToInt32(LogTool.LogLevel) < 1)
            {
                LogTool.LogLevel = eloglevel.error; //errors show at the minimum.
            }
            else
            {
                LogTool.LogLevel = logLevel;
            }
            string[] fileDefaults = DefaultLogToolAndLocation();
            if (logFileName == string.Empty)
                LogTool.LogName = fileDefaults[1];
            else
                LogTool.LogName = logFileName;

            if (logLocation == string.Empty)
                LogTool.LogBasePath = fileDefaults[0];
            else
                LogTool.LogBasePath = logLocation;

            //evaluate the quota size as meaning kbytes.
            LogTool.LogQuotaFormat = elogquotaformat.kbytes;
            //So, max logfile size = (Log.LogQuotaFormat * Log.LogSizeMax)
            LogTool.LogSizeMax = 100;
            //Every Log.LogPeriod the log file will roll over to a new file.
            LogTool.LogPeriod = elogperiod.day;
            //The log filename will be formatted this way.
            LogTool.LogNameFormat = elognameformat.date_name;
            LogTool.SetLogPath();
            /*
        ...
        private static string[] DefaultLogToolAndLocation()
        {
        ...
        public static void LogHeader(elogprofile logprofile, string sText, elogheaderlevel logheaderlevel)
        {
            ...
            switch (logheaderlevel)
            {
                case elogheaderlevel.Level_1:
                    sHeader = "======(" + System.AppDomain.CurrentDomain.FriendlyName + ") ====  " + sText + "============ Date:" + dt.ToString("yyyyMMdd") + " Time:" + dt.ToLongTimeString();
                    break;
    ...
    public class LogMethods //: System.IDisposable
    {
        ...
        public void TruncateLogTool()
        {
            TruncateLogTool(LogPath);
        }
        public void TruncateLogTool(string sLogPath)
        {
            ...
            if (m_loglevel >= loglevel)
            {
                ...
                switch (logprefix)
                {
                    ...
                    case elogprefix.dt_loglevel:                       
                        logtext = dt.ToString("yyyy.MM.dd") + "-" + dt.ToLongTimeString() + ">>>" + loglevel + ": " + logtext;
                        break;
                }
