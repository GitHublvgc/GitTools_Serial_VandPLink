...
namespace VandP_Serial
{
    public class SerialTool
    {
       ...
       // private long received_count = 0;//接收计数
        ...
        //Common.Tools.MTEs.tabPageSetting TabPageSetting = new tabPageSetting();
        ...
        /// <param name="dataBits">数据长度</param>
        /// <param name="stopBits">停止位</param>
        public bool setSerialPort(string comPortName, int baudRate, string parity,int dataBits, string stopBits)
        {
            if (_serialPort.IsOpen)
                _serialPort.Close();
        ...
        public  byte[] SendCommand(byte[] SendData,int Sendlen, int Overtime)
        {
            ...
            try
            {
                ...
                byte[] globalArray = new byte[4096];
                int num = 0, ret = 0, count = 0,dataNum=0;
                //System.Threading.Thread.Sleep(TabPageSetting.m_tagSettingPower.iRS485ResendInterval);
                ReceiveEventFlag = true;      //关闭事件
                int relayTime = Overtime / 100 + 1;
                while (num++ < relayTime)
                {
                    
                    while (_serialPort.IsOpen && ((count = _serialPort.BytesToRead) != 0))
                    {
                        
                        byte[] bufferB = new byte[count];
                        _serialPort.Read(bufferB, 0, count);
                        Array.Copy(bufferB, 0, globalArray, dataNum, count);
                        dataNum += count;
                    ...
                    if (dataNum != 0)
                    {
                        if (globalArray != null)
                        {
                            #region 排除低端红外头自发自收报文  注释掉
                            string sendStr = byteToHexStr(SendData);
                            string receStr = byteToHexStr(globalArray, 0, dataNum);
                            if (receStr.Contains(sendStr))
                            {
                                receStr = receStr.Remove(0, sendStr.Length);
                                byte[] tt = strToToHexByte(receStr);
                                dataNum = dataNum - SendData.Length;
                                Array.Copy(tt, 0, globalArray, 0, dataNum);
                            }
                            #endregion
                            byte[] retArray=new byte[dataNum];
                            Array.Copy(globalArray, 0, retArray, 0, dataNum);
                            return retArray;
                        }
                    }
        ...
        public byte[] fnSyncRead(int Overtime)
        {
            try
            {
                if (!_serialPort.IsOpen)
        ...
        public static string byteToHexStr(byte[] bytes,int startIndex,int len)
        {
            ...
            if (bytes != null)
            {
                ...
                for (int i = 0; i < len; i++)
                {
                    returnStr += bytes[startIndex + i].ToString("X2");
                }
            }
            return returnStr;
        }
